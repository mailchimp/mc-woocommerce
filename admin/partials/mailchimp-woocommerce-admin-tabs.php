...
if ( ! $mc_configured ) {
	if ( $active_tab == 'sync' || $active_tab == 'logs' ) {
				...
				<div class="mc-woocommerce-settings-header">
					...
					<div class="mc-woocommerce-settings-subtitles">
						...
						<?php if ( $active_tab == 'api_key' ) : ?>
							...
							<span class="mc-woocommerce-header-title"> <?php echo wp_kses( translate( 'Add Mailchimp for WooCommerce', 'mailchimp-for-woocommerce' ), $allowed_html ); ?> </span>
							<span class="mc-woocommerce-header-subtitle"> <?php echo wp_kses( translate( 'Build custom segments, send automations, and track purchase <br/>activity in Mailchimp. Login to authorize an account connection.', 'mailchimp-for-woocommerce' ), $allowed_html ); ?> </span>
						<?php endif; ?>
				
						<?php if ( $active_tab == 'store_info' && $has_valid_api_key ) : ?>
							<span class="mc-woocommerce-header-steps">2 of 3 - Store</span>    
							<span class="mc-woocommerce-header-title"> <?php echo wp_kses( translate( 'Add WooCommerce store settings', 'mailchimp-for-woocommerce' ), $allowed_html ); ?> </span>
							<span class="mc-woocommerce-header-subtitle"> <?php echo wp_kses( translate( 'Please provide a bit of information about your WooCommerce <br/> store and location.', 'mailchimp-for-woocommerce' ), $allowed_html ); ?> </span>
						<?php endif; ?>
				
						<?php if ( $active_tab == 'newsletter_settings' ) : ?>
							<span class="mc-woocommerce-header-steps">3 of 3 - Audience</span>    
							<span class="mc-woocommerce-header-title"> <?php echo wp_kses( translate( 'Add WooCommerce audience settings', 'mailchimp-for-woocommerce' ), $allowed_html ); ?> </span>
							<span class="mc-woocommerce-header-subtitle">
								<?php echo wp_kses( translate( 'Please provide a bit of information about your WooCommerce <br/> campaign and messaging settings.', 'mailchimp-for-woocommerce' ), $allowed_html ); ?>
								<?php
								if ( ! $only_one_list ) {
									echo wp_kses( translate( 'If you don’t have an audience, <br/> you can choose to create one', 'mailchimp-for-woocommerce' ), $allowed_html );}
								?>
							</span>
						<?php endif; ?>    
					</div>
					...
					<div class="mc-woocommerce-wizard-btn">
						<?php if ( $active_tab == 'store_info' && $has_valid_api_key ) : ?>
								<?php submit_button( __( 'Next step', 'mailchimp-for-woocommerce' ), 'primary tab-content-submit', 'mailchimp_submit' ); ?>
								<a href="?page=mailchimp-woocommerce&tab=api_key" class="button button-default back-step"><?php echo __( 'Back', 'mailchimp-for-woocommerce' ); ?></a>
						<?php endif; ?>

						<?php if ( $active_tab == 'newsletter_settings' && $has_valid_api_key ) : ?>
								<?php submit_button( __( 'Start sync', 'mailchimp-for-woocommerce' ), 'primary tab-content-submit', 'mailchimp_submit' ); ?>
								<a href="?page=mailchimp-woocommerce&tab=store_info" class="button button-default back-step"><?php echo __( 'Back', 'mailchimp-for-woocommerce' ); ?></a>
						<?php endif; ?>
					</div>
				</div>
			</div>
				...
				<div class="mc-woocommerce-settings-header">
					...
					<p class="mc-woocommerce-settings-subtitles">
						...
						if ( $active_tab == 'api_key' ) {
							echo wp_kses( translate( 'Add Mailchimp for WooCommerce to build custom segments, send automations, and track purchase activity in Mailchimp', 'mailchimp-for-woocommerce' ), $allowed_html );
						}

						if ( $active_tab == 'store_info' && $has_valid_api_key ) {
							if ( $show_sync_tab ) {
								echo wp_kses( translate( 'WooCommerce store and location', 'mailchimp-for-woocommerce' ), $allowed_html );
							} else {
								echo wp_kses( translate( 'Please provide a bit of information about your WooCommerce store', 'mailchimp-for-woocommerce' ), $allowed_html );
							}
						}

						if ( $active_tab == 'newsletter_settings' ) {
							if ( $show_sync_tab ) {
								echo wp_kses( translate( 'Campaign and messaging settings', 'mailchimp-for-woocommerce' ), $allowed_html );
							} else {
								if ( $only_one_list ) {
									echo wp_kses( translate( 'Please apply your audience settings.', 'mailchimp-for-woocommerce' ), $allowed_html );
								} else {
									echo wp_kses( translate( 'Please apply your audience settings. ', 'mailchimp-for-woocommerce' ), $allowed_html );
									echo wp_kses( translate( 'If you don’t have an audience, you can choose to create one', 'mailchimp-for-woocommerce' ), $allowed_html );
								}
							}
						}
						...
						if ( $active_tab == 'sync' && $show_sync_tab ) {
							if ( mailchimp_is_done_syncing() ) {
								echo wp_kses( translate( 'Success! You are connected to Mailchimp', 'mailchimp-for-woocommerce' ), $allowed_html );
							} else {
								echo wp_kses( translate( 'Your WooCommerce store is syncing to Mailchimp', 'mailchimp-for-woocommerce' ), $allowed_html );
							}
						}

						if ( $active_tab == 'logs' && $show_sync_tab ) {
							echo wp_kses( translate( 'Log events from the Mailchimp plugin', 'mailchimp-for-woocommerce' ), $allowed_html );
						}

						if ( $active_tab == 'plugin_settings' && $show_sync_tab ) {
							echo wp_kses( translate( 'Connection settings', 'mailchimp-for-woocommerce' ), $allowed_html );
						}
						?>
					</p>
					<div class="nav-tab-wrapper">
						<?php if ( $has_valid_api_key ) : ?>
							<?php if ( $active_tab == 'api_key' ) : ?>
								<a href="?page=mailchimp-woocommerce&tab=api_key" class="nav-tab <?php echo $active_tab == 'api_key' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Connect', 'mailchimp-for-woocommerce' ); ?></a>
							<?php endif; ?>
							<a href="?page=mailchimp-woocommerce&tab=sync" class="nav-tab <?php echo $active_tab == 'sync' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Overview', 'mailchimp-for-woocommerce' ); ?></a>
							<a href="?page=mailchimp-woocommerce&tab=store_info" class="nav-tab <?php echo $active_tab == 'store_info' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Store', 'mailchimp-for-woocommerce' ); ?></a>
							<?php if ( $handler->hasValidStoreInfo() ) : ?>
									<a href="?page=mailchimp-woocommerce&tab=newsletter_settings" class="nav-tab <?php echo $active_tab == 'newsletter_settings' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Audience', 'mailchimp-for-woocommerce' ); ?></a>
							<?php endif; ?>
							<a href="?page=mailchimp-woocommerce&tab=logs" class="nav-tab <?php echo $active_tab == 'logs' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Logs', 'mailchimp-for-woocommerce' ); ?></a>
							<a href="?page=mailchimp-woocommerce&tab=plugin_settings" class="nav-tab <?php echo $active_tab == 'plugin_settings' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Settings', 'mailchimp-for-woocommerce' ); ?></a>
						<?php endif; ?>
					</div>
				</div>
			</div>
		<?php endif; ?>
		<div class="notices-content-wrapper <?php echo $active_tab == 'sync' ? 'sync-notices' : ''; ?>">
		<?php
			$settings_errors = get_settings_errors();
		if ( ! $show_wizard || ( isset( $settings_errors[0] ) && $settings_errors[0]['type'] != 'success' ) ) {
