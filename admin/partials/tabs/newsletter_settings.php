...
if ( ! $handler->validateApiKey() || ( ! isset( $mailchimp_lists ) && ( $mailchimp_lists = $handler->getMailChimpLists() ) === false ) ) {
...
$list_is_configured = isset( $options['mailchimp_list'] ) &&
                      ( ! empty( $options['mailchimp_list'] ) ) &&
                      array_key_exists( $options['mailchimp_list'], (isset($mailchimp_lists) ? $mailchimp_lists : array()) );
...
<fieldset>
	...
	<?php if ( ! $list_is_configured ) : ?>
		...
		<div class="box" >
			...
			<div class="mailchimp-select-wrapper">
				<select name="<?php echo $this->plugin_name; ?>[mailchimp_list]" required <?php echo (isset($only_one_list) && $only_one_list) ? 'disabled' : ''; ?>>
	...
	<?php else : ?>
		...
		<div class="box" >
			<p><?php echo __( 'To select another audience, you must first disconnect your store on the Settings tab.', 'mailchimp-for-woocommerce' ); ?></p>
		</div>
	<?php endif; ?>
	...
	<div class="box box-half">
		...
		<input type="text" id="<?php echo $this->plugin_name; ?>-campaign-from-name-label" name="<?php echo $this->plugin_name; ?>[campaign_from_name]" value="<?php echo esc_html((isset( $options['campaign_from_name'] ) ? $options['campaign_from_name'] : get_option( 'blogname' ))); ?>" required/>
	</div>
	...
	<div class="box box-half">
		...
		<input type="email" id="<?php echo $this->plugin_name; ?>-campaign-from-email-label" name="<?php echo $this->plugin_name; ?>[campaign_from_email]" value="<?php echo esc_html((isset( $options['campaign_from_email'] ) ? $options['campaign_from_email'] : get_option( 'admin_email' ))); ?>" required/>
	</div>
	...
	<div class="box box-half">
		...
		<input type="text" id="<?php echo $this->plugin_name; ?>-campaign-subject-label" name="<?php echo $this->plugin_name; ?>[campaign_subject]" value="<?php echo isset( $options['campaign_subject'] ) ? esc_html($options['campaign_subject']) : esc_html__( 'Store Newsletter', 'mailchimp-for-woocommerce' ); ?>" required/>
	</div>
	...
	<div class="box">
		...
		<textarea
			...
			required><?php echo isset( $options['campaign_permission_reminder'] ) ? esc_html($options['campaign_permission_reminder']) : sprintf( /* translators: %s - plugin name. */esc_html__( 'You were subscribed to the newsletter from %s', 'mailchimp-for-woocommerce' ), get_option( 'blogname' ) ); ?>
		</textarea>
	</div>
	...
	<div class="optional-settings-content">
		...
		<div class="box" >
			<label>
				...
				<p><?php echo __( 'WooCommerce does not keep a historical record of a customer\'s opt-in preferences. This plugin will add this functionality moving forward. Do you want to subscribe all customers to your Mailchimp Audience or only those moving forward?', 'mailchimp-for-woocommerce' ); ?></p>
			</label>
			<div class="box margin-large"></div>
			<label class="radio-label">
				<input type="radio" name="<?php echo $this->plugin_name; ?>[mailchimp_auto_subscribe]" value="1"
													 <?php
														if ( (bool) $enable_auto_subscribe ) {
															echo ' checked="checked" ';}
														?>
				><?php esc_html_e( 'Auto-subscribe all customers on the initial sync', 'mailchimp-for-woocommerce' ); ?><br>
			</label>
			<label class="radio-label">
				<input type="radio" name="<?php echo $this->plugin_name; ?>[mailchimp_auto_subscribe]" value="0"
													 <?php
														if ( ! (bool) $enable_auto_subscribe ) {
															echo ' checked="checked" ';}
														?>
				><?php esc_html_e( 'Only sync customers in this store who are subscribed in my Mailchimp Audience', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
		</div>
		<?php endif; ?>
		...
		<div class="box" >
			...
			<label>
				...
				<p><?php echo __( 'To maximize your marketing efforts, we recommend syncing all customer data with Mailchimp. This way, you can send both marketing and transactional campaigns to your clients. You also have the option to only sync those who subscribe to your Audience at checkout or create an account on your store. Transactional messages like abandoned carts, order confirmations, etc., will not trigger if you decide to only send to subscribers.', 'mailchimp-for-woocommerce' ); ?></p>
			</label>
			<div class="box margin-large"></div>
			<label class="radio-label">
				<input type="radio" id="ongoing_sync_all" name="<?php echo $this->plugin_name; ?>[mailchimp_ongoing_sync_status]" value="1"
																		   <?php
																			if ( (bool) $ongoing_sync_subscribe ) {
																				echo ' checked="checked" ';}
																			?>
				><?php esc_html_e( 'Send all data to your Mailchimp Audience, including subscribers and transactional customers', 'mailchimp-for-woocommerce' ); ?><br>
			</label>
			<label class="radio-label">
				<input type="radio" id="ongoing_sync_subscribed" name="<?php echo $this->plugin_name; ?>[mailchimp_ongoing_sync_status]" value="0"
																				  <?php
																					if ( ! (bool) $ongoing_sync_subscribe ) {
																						echo ' checked="checked" ';}
																					?>
				><?php esc_html_e( 'Only sync subscribers. No transactional customers will be sent to your Mailchimp Audience', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
		</div>
		...
		<div class="box" >
			...
			<label>
				...
				<p><?php echo __( 'You may disable this if you are not using Abandoned Cart Automations.', 'mailchimp-for-woocommerce' ); ?></p>
			</label>
			<div class="box margin-large"></div>
			<label class="radio-label">
				<input type="radio" id="cart_track_all" name="<?php echo $this->plugin_name; ?>[mailchimp_cart_tracking]" value="all"
																		 <?php
																			if ( $mailchimp_cart_tracking === 'all' ) {
																				echo ' checked="checked" ';}
																			?>
				><?php esc_html_e( 'Track carts for all customers', 'mailchimp-for-woocommerce' ); ?><br>
			</label>
			<label class="radio-label">
				<input type="radio" id="cart_track_subscribed" name="<?php echo $this->plugin_name; ?>[mailchimp_cart_tracking]" value="subscribed"
																				<?php
																				if ( $mailchimp_cart_tracking === 'subscribed' ) {
																					echo ' checked="checked" ';}
																				?>
				><?php esc_html_e( 'Only track carts for subscribed customers', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
			<label class="radio-label">
				<input type="radio" id="cart_track_none" name="<?php echo $this->plugin_name; ?>[mailchimp_cart_tracking]" value="disabled"
																		  <?php
																			if ( $mailchimp_cart_tracking === 'disabled' ) {
																				echo ' checked="checked" ';}
																			?>
				><?php esc_html_e( 'Disable cart tracking', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
		</div>
		...
		<div class="box box-half">
			...
			<label class="radio-label">
				<input type="radio" name="<?php echo $this->plugin_name; ?>[mailchimp_checkbox_defaults]" value="check"
													 <?php
														if ( $checkbox_default_settings === 'check' ) {
															echo ' checked="checked" ';}
														?>
				><?php esc_html_e( 'Visible, checked by default', 'mailchimp-for-woocommerce' ); ?><br>
			</label>
			<label class="radio-label">
				<input type="radio" name="<?php echo $this->plugin_name; ?>[mailchimp_checkbox_defaults]" value="uncheck"
													 <?php
														if ( $checkbox_default_settings === 'uncheck' ) {
															echo ' checked="checked" ';}
														?>
				><?php esc_html_e( 'Visible, unchecked by default', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
			<label class="radio-label">
				<input type="radio" name="<?php echo $this->plugin_name; ?>[mailchimp_checkbox_defaults]" value="hide"
													 <?php
														if ( $checkbox_default_settings === 'hide' ) {
															echo ' checked="checked" ';}
														?>
				><?php esc_html_e( 'Hidden, unchecked by default', 'mailchimp-for-woocommerce' ); ?><br/>
			</label>
			<div class="box margin-large"></div>
		...
		<div class="box box-half">
			<input type="text" id="<?php echo $this->plugin_name; ?>-newsletter-checkbox-action" name="<?php echo $this->plugin_name; ?>[mailchimp_checkbox_action]" value="<?php echo isset( $options['mailchimp_checkbox_action'] ) ? esc_html($options['mailchimp_checkbox_action']) : 'woocommerce_after_checkout_billing_form'; ?>" />
		...
		<div class="box box-half" >
			<label for="<?php echo $this->plugin_name; ?>-user-tags">
				<p><?php echo __( 'Add a comma-separated list of tags to apply to a subscriber in Mailchimp after a transaction occurs', 'mailchimp-for-woocommerce' ); ?></p>
			</label>
		</div>

		<div class="box box-half" >
			<input type="text" id="<?php echo $this->plugin_name; ?>-user-tags" name="<?php echo $this->plugin_name; ?>[mailchimp_user_tags]" value="<?php echo isset( $options['mailchimp_user_tags'] ) ? str_replace( ',', ', ', esc_html($options['mailchimp_user_tags']) ) : ''; ?>" />
		</div>
		...
		<div class="box box-half">
			<label for="<?php echo $this->plugin_name; ?>[mailchimp_product_image_key]">
				<p><?php echo __( 'Define the product image size used by abandoned carts, order notifications, and product recommendations.', 'mailchimp-for-woocommerce' ); ?></p>
			</label>
		</div>
