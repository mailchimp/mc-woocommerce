...
class MailChimp_Newsletter extends MailChimp_WooCommerce_Options
{
    ...
    public function applyNewsletterField($checkout)
    {
        if (!is_admin()) {
            ...
            $checkbox .= '<label for="mailchimp_woocommerce_newsletter" class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox"><span>' . __($label, 'mailchimp-woocommerce') . '</span></label>';
            $checkbox .= '</p>';
