...
class MailChimp_WooCommerce_Subscriber_Sync extends Mailchimp_Woocommerce_Job
{
    ...
    /**
     * Handle SMS subscribe/unsubscribe webhook events from Mailchimp
     *
     * @param string $hook_type The webhook event type (sms_subscribe or sms_unsubscribe)
    ...
    private function handleSmsEvent($hook_type, $data, $email)
    {
        ...
        if (isset($data['sms_phone_number'])) {
            $sms_phone = $data['sms_phone_number'];
        } elseif (isset($data['sms_channel']['sms_phone'])) {
            $sms_phone = $data['sms_channel']['sms_phone'];
        }
        ...
        try {
            ...
            if ($handled === $sms_subscribe) {
                mailchimp_log('webhook.sms', "SMS sync already handled for {$email}");
                return true;
            }
